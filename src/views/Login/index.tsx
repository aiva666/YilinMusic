 ...
 * @LastEditors: Aiva
 * @LastEditTime: 2021-12-21 13:27:11
...
const Login = (props: any) => {
    ...
    return (
        <Form onFinish={commitForm}>
            <Form.Item name="phone" rules={telRules}>
                <Input
                    type="tel"
                    placeholder="手机号码"
                    allowClear
...
const LoginBox: FC = () => {
    ...
    const loginCommit = async (val: object) => {
        let res: any = await login({ params: val });
        if (res) {
            const { token, profile, cookie, bindings, account } = res;
